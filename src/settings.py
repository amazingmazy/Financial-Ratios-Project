...
## Dates
# START_DATE is 1926 rather than the template's 1913 because that is where CRSP
# begins. The replication itself only needs 1946 onward (Lewellen omits the
# Depression era deliberately -- Section 3 -- because 1930s volatility distorts
# both the variance and the persistence of dividend yield), but pulling from
# 1926 gives the pre-sample months the trailing-12-month dividend window needs
# and leaves room for pre-1946 robustness checks.
defaults["START_DATE"] = datetime.strptime("1926-01-01", "%Y-%m-%d")
defaults["END_DATE"] = datetime.strptime("2025-12-31", "%Y-%m-%d")

# CRSP's legacy SIZ tables stopped being updated after 2024-12-31, while the
# current CIZ schema runs a year further. The replication is built on SIZ (its
# month-end return compounding matches the convention Lewellen's data used and
# reproduces his numbers more closely) and the extension on CIZ, so the two
# pulls need different end dates and this one cannot simply be END_DATE.
# See ISSUE2.md, "Known gaps: legacy CRSP tables are frozen at 2024".
defaults["SIZ_END_DATE"] = datetime.strptime("2024-12-31", "%Y-%m-%d")

# Monte Carlo draws for the Stambaugh bias correction. At 2,000 draws two seeds
# gave 0.2508 and 0.2851 for the same slope, a spread of 0.034, so anything
# reported needs a higher count than a quick look does.
defaults["N_SIMS"] = 8000
